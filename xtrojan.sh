...
if [[ $(/usr/bin/id -u) -ne 0 ]]; then
  echo "¡No eres un usuario root! Introduzca sudo -i para cambiar al usuario root antes de ejecutar este script "
  exit
fi
...
build_web() {
  if [ ! -f "/var/www/html/index.html" ]; then
    ...
    mkdir -p /var/www/html/xwall
    unzip -q /tmp/template.zip -d /var/www/html/xwall
    echo -ne "User-agent: *\nDisallow: /\n" > /var/www/html/xwall/robots.txt
  else
...
show_links() {
  if [ -f "/usr/local/bin/xray" ]; then
    ...
    colorEcho ${YELLOW} "Debido a que el formato del enlace para compartir de Trojan-Go no se ha finalizado, si su cliente no puede analizar este enlace, complete la información de conexión manualmente "
    printf "%s:443 %s %s\n" "${sni}" "${uuid}" "${path}tj"
    echo ""
...
rm_xwall() {
  if [ -f "/usr/local/bin/xray" ]; then
    wget -q https://${raw_proxy}/ThonyDroidYT/v2ray-tcp-tls-web/${branch}/tools/rm_xwall.sh -O /tmp/rm_xwall.sh && bash /tmp/rm_xwall.sh
    exit 0
  fi
}
...
menu() {
  ...
  colorEcho ${YELLOW} "Author: phlinhng/ThonyDroidYT"
